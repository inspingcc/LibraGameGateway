...
public class MathUtils {
	...
	public static int random(int size) {
		try {
			...
			//GameLog.error("random size="+size);
			return 0;
		}
	}
	...
	public static boolean randomLessThan(int chn) {
		return chn >0 && RND.nextInt(100) < chn;
	}
	...
	public static <T> List<T> randoms(Collection<T> ts, int maxCount) {
		...
		if (ts.size() <= maxCount) return new ArrayList<>(ts);
		if (maxCount == 0) return Collections.emptyList();

		List<T> result = new ArrayList<>(maxCount);
...
//	}
	...
	public static <K, V> List<V> randomMapValues(Map<K, V> map, int num) {
		...
		int size = map.size();
		if (size <= num) return new ArrayList<>(map.values());
		//随机IDX
		List<K> randkeys = randoms(map.keySet(), num);
		List<V> vals = new ArrayList<>(num);
		for (K k : randkeys) {
			vals.add(map.get(k));
		}
		return vals;
